...
class Tipoempresa {
   ...
   public static function insertar($data = []) {
      if (!empty($data)) {
         $sql = "INSERT INTO t_tipo_empresa(tipo_empresa ) VALUES(:tipo_empresa )";
         try {
   ...
   public static function listar() {
      $sql = 'SELECT * FROM t_tipo_empresa;';
      try {
         $con = \Core\Model::getInstance();
   ...
   public static function editar($data = []) {
      if (!empty($data)) {
         $sql = "UPDATE t_tipo_empresa SET tipo_empresa  = :tipo_empresa  WHERE id = :id";
         try {
            $con = \Core\Model::getInstance();
            $result = $con->consulta($sql, $data);
            if (!empty($result)) {
   ...
   public static function eliminar($data = []) {
      if (!empty($data)) {
         $sql = 'DELETE FROM t_tipo_empresa WHERE id = :id';
         try {
            $con = \Core\Model::getInstance();
            $result = $con->consulta($sql, $data);
            if (!empty($result)) {
