...
class Tipoconsulta{
   public static function insertar($data = []){
		if(!empty($data)){
			$sql = "INSERT INTO t_tipo_consulta(tipo_consulta) VALUES(:tipo_consulta)";
			try {
	...
	public static function listar(){
		$sql = 'SELECT * FROM t_tipo_consulta;';
		try {
			$con = \Core\Model::getInstance();
	...
	public static function editar($data = []){
		if(!empty($data)){
			$sql = "UPDATE t_tipo_consulta SET tipo_consulta = :tipo_consulta WHERE id = :id";
			try {
				$con = \Core\Model::getInstance();
				$result = $con->consulta($sql, $data);
				if (!empty($result)) {
	...
	public static function eliminar($data = []){
		if(!empty($data)){
			$sql = 'DELETE FROM t_tipo_consulta WHERE id = :id';
			try {
				$con = \Core\Model::getInstance();
				$result = $con->consulta($sql, $data);
				if (!empty($result)) {
