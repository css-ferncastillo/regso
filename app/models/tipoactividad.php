...
class Tipoactividad{
   public static function insertar($data = []){
		if(!empty($data)){
			$sql = "INSERT INTO t_tipo_actividad (tipo_actividad) VALUES(:tipo_actividad)";
			try {
	...
	public static function listar(){
		$sql = 'SELECT * FROM t_tipo_actividad ;';
		try {
			$con = \Core\Model::getInstance();
	...
	public static function editar($data = []){
		if(!empty($data)){
			$sql = "UPDATE t_tipo_actividad  SET tipo_actividad = :tipo_actividad WHERE id = :id";
			try {
				$con = \Core\Model::getInstance();
				$result = $con->consulta($sql, $data);
				if (!empty($result)) {
	...
	public static function eliminar($data = []){
		if(!empty($data)){
			$sql = 'DELETE FROM t_tipo_actividad  WHERE id = :id';
			try {
				$con = \Core\Model::getInstance();
				$result = $con->consulta($sql, $data);
				if (!empty($result)) {
